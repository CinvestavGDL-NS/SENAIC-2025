...
species person skills: [moving]
{
	...
	// Reflejo para interactuar con otros agentes y hacer amistad.
	reflex interact_with_others when: state = "in_place"{
		if interaction_intervention = true{
			interaction_prob 	<- 0.2;
		}
		else{
			interaction_prob 	<- 0.1;
		}
		ask person at_distance(25){
				if  flip(interaction_prob){
					myself.social_fabric <- myself.social_fabric + 1;
					self.social_fabric <- self.social_fabric + 1;
				}
			}
	}
	
	// Reflejo para evaluar vender o no vender (salir de la comunidad)
	reflex evaluate_selling when: every(100 #cycle){
		if frustration > social_fabric{
			considering_selling <- true;
		}
		else{
			considering_selling <- false;
		}
		social_fabric <- int(social_fabric/4);
		frustration   <- int(frustration/3);
...
experiment main type:gui
{
	...
	output synchronized: true
	{
		...
		display "Proceso de gentrificación" type: 2d{
			chart "Posible Gentrificación" type:series 
			x_serie_labels:("cycle"+cycle)
			{
				data "posibles ventas" 	value: length(person where (each.considering_selling = true))
					color:#deeppink;
			}
		}
